...
summary: "A decision guide for teams evaluating C# for backend services, enterprise systems, desktop applications, Unity games, .NET-centered tooling, F# domain cores, and Visual Basic or Delphi modernization decisions."
audience: Developers and teams deciding whether C# should be their main language for .NET services, business applications, desktop tools, Unity projects, Microsoft-centered platforms, mixed C#/F# systems, or modernization work near existing Visual Basic and Delphi systems.
candidates:
  - csharp
  - fsharp
  - java
...
If the application is already a Visual Basic, Delphi, or VCL product, compare C# against modernization in place. C# may be the better destination for new .NET services, Windows tools, or cross-team platform consolidation, but existing VB6, VB.NET, VBA, and Delphi desktop applications often carry form resources, component dependencies, COM references, database bindings, reports, installers, documents, and user workflows that should be stabilized before migration. See [Visual Basic vs C#](/comparisons/visual-basic-vs-csharp) and [Delphi vs C#](/comparisons/delphi-vs-csharp) for focused comparisons.
...
## Consider F# For Functional .NET Cores

Consider [F#](/languages/fsharp) when the platform should remain .NET but the hardest code is domain modeling, validation, calculations, rules, data transformation, or workflow composition. F# brings records, discriminated unions, option/result-oriented modeling, pattern matching, pipelines, type inference, scripts, and F# Interactive while still using NuGet, .NET deployment, and C# interoperability.

This does not make F# a replacement for C# across every .NET surface. C# is usually the smoother default for ASP.NET Core templates, desktop UI, Unity, vendor samples, broad hiring, Roslyn analyzers, and framework-heavy glue. A mixed solution can work well when C# owns framework boundaries and F# owns typed domain cores, but the team must standardize project references, async/task interop, nullable boundaries, formatting, and testing.
...
Use F# alongside C# when the same .NET platform needs a more functional domain layer or data-transformation layer.
...
Keep Delphi in the decision when the durable asset is an existing VCL, FireMonkey, Object Pascal, or RAD Studio application. A maintained Delphi system can be the right answer when the alternative is an under-specified rewrite.

Keep Visual Basic in the decision when the durable asset is an existing VB.NET, VB6, VBA, Windows Forms, COM, or Office automation system. C# is usually the better home for new .NET work, but Visual Basic behavior should be inventoried and tested before it is replaced.
